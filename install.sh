...
echo 'alias fire="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/xianfeic/fire" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/xianfeic/fire'"
...
rm -rf "$HOME/.config/@fire"
rm -rf "$HOME/.config/@fire"
rm -rf "$HOME/.config/fire-core"
